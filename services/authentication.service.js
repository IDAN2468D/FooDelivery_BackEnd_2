...
const sendVerificationEmail = async (email) => {
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            ...
            pass: 'sqkqcgndpdpdgujx'
        }
    });

    const mailOptions = {
        from: 'kazam11@netvision.net',
        to: email,
...
const checkUserExist = async (query) => {
    ...
    try {
        ...
        let userObject = await MongoDB.db
            ...
            .findOne(query);
        return !userObject
            ? { status: true, message: `This ${queryType} is taken` }
...
const tokenRefresh = async (req, res) => {
    ...
    try {
        ...
        if (token && token.startsWith("Bearer")) {
            token = token.slice(7, token?.length)
            jwt.verify(token, tokenSecret, { ignoreExpiration: true }, async (error, decoded) => {
                if (error) {
                    res.status(401).json({
                        status: false,
                        message: error?.name ? error?.name : "Invalid Token",
